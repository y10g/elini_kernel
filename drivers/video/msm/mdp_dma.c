 ...
 */
    // ALESSI MUST HAVE !! bob.cho@lge.com
...
/* LGE_CHANGE
  ...
  */
	...
	//	EPRINTK("%s:  mddi_hitachi_2c started.\n", __func__);
...
void mdp_dma_pan_update(struct fb_info *info)
{
	...
	if (is_display_on == 1)
	{
		...
		MSM_FB_INFO("00000mdp_dma_pan_update____is_display_on \n");
	}
	if (mfd->sw_currently_refreshing) {
		MSM_FB_INFO("11111mdp_dma_pan_update: \n");
		...
		MSM_FB_INFO("22222mdp_dma_pan_update: \n");
		/*LGE_CHANGE_E [bluerti@lge.com] */
	} else
		mfd->dma_fnc(mfd);

		MSM_FB_INFO("00000mdp_dma_pan_update: \n");
}
