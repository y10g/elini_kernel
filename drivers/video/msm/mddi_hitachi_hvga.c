...
static struct display_table mddi_hitachi_display_on_3rd[] = {
	...
	{0x11, 4, {0x00, 0x00, 0x00, 0x00}},
	{REGFLAG_DELAY, 80, {}},
//	{0x29, 4, {0x00, 0x00, 0x00, 0x00}}, for test
	{REGFLAG_END_OF_TABLE, 0x00, {}}
};
static struct display_table mddi_hitachi_display_on_3rd_elt[] = {
	// Display on sequence
	{0x11, 4, {0x00, 0x00, 0x00, 0x00}},
	{REGFLAG_DELAY, 80, {}},
	{0x29, 4, {0x00, 0x00, 0x00, 0x00}}, 
    ...
    for(i = 0; i < count; i++) {
        ...
        switch (reg) {
            ...
            case REGFLAG_DELAY :
				...
				EPRINTK("[Choongho]%s() : delay %d msec\n", __func__, table[i].count);
                break;
...
//				if(table[i].val_list != temp)
				...
				EPRINTK("[Choongho]_%s: reg : %x, val : %x.\n", __func__, reg, table[i].val_list[0]);
       	}
    }	
}
...
#if defined(CONFIG_MACH_MSM7X27_THUNDERG) || defined(CONFIG_MACH_MSM7X27_THUNDERC) || defined(CONFIG_MACH_MSM7X27_ELINI)
//LGE_CHANGE LCD Upate Error choongho.kim@lge.com 
...
//	else
	{
		display_table(mddi_hitachi_initialize_3rd, sizeof(mddi_hitachi_initialize_3rd)/sizeof(struct display_table));
		mdelay(200);
		display_table(mddi_hitachi_display_on_3rd_elt, sizeof(mddi_hitachi_display_on_3rd_elt) / sizeof(struct display_table));
	}
#endif
	is_lcd_on = TRUE;
	return 0;
...
static void mddi_hitachi_lcd_panel_poweron(void)
{
	...
	//	mdelay(10);
		...
		mdelay(10);
		EPRINTK("%s: [Choongho]_mdelay(10).\n", __func__);
		gpio_set_value(pdata->gpio, 1);
		mdelay(20);
		EPRINTK("%s: [Choongho]_mdelay(20).\n", __func__);
	}
}
