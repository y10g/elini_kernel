...
#if ! defined(CONFIG_MACH_MSM7X27_ELINI) && !defined(CONFIG_MACH_MSM7X27_ALESSI)
...
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/08/21,<sp의 SR적용 > Elini]*/
#define TS_POLLING_TIME 10//0//5 /* msec */
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/08/21,< > Elini]*/
...
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/09/18,<for touch frimware info > Elini]*/
void mcs6000_info(unsigned char* fw_ver, unsigned char* hw_ver)
{
	struct mcs6000_ts_device *dev = NULL;	
	dev = &mcs6000_ts_dev;
      msleep(100);
  *fw_ver= i2c_smbus_read_byte_data(dev->client, MCS6000_TS_FW_VERSION);
   msleep(10);
  *hw_ver =i2c_smbus_read_byte_data(dev->client, MCS6000_TS_HW_REVISION);
}

EXPORT_SYMBOL(mcs6000_info);
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/09/18,< > Elini]*/
		...
		case MCS6000_TS_IOCTL_MAGIC :
			switch(cmd){
				...
				case MCS6000_TS_IOCTL_MAIN_OFF:
					break;
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/09/18,< > Elini]*/					
			       case MCS6000_TS_IOCTL_INFO:
				   	{
					unsigned char fw_ver, hw_ver;
					printk(KERN_INFO "\n==== mcs6000_infol....%d\n",_IOC_NR(cmd));
					mcs6000_info(&fw_ver, &hw_ver);
					err = fw_ver;
					break;
				     }
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/09/18,< > Elini]*/				   	
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/09/27,< > Elini]*/
			       case MCS6000_TS_IOCTL_HW_INFO:
				   	{
					unsigned char fw_ver, hw_ver;
					printk(KERN_INFO "\n==== mcs6000_infol....%d\n",_IOC_NR(cmd));
					mcs6000_info(&fw_ver, &hw_ver);
...
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/08/21,< > Elini]*/	
	ts_early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN +1;//- 40;
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/08/21,< > Elini]*/
	register_early_suspend(&ts_early_suspend);
#endif
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/09/17,<lg이노텍 touch 사용위해.> Elini]*/
/*
if(lge_bd_rev>=LGE_REV_D) //for M2 Auto update
{
	mcs6000_firmware_info(&fw_ver, &hw_ver);
}
*/
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/09/17,< > Elini]*/	
...
// LGE_CHANGE_S dangwoo.choi@lge.com - Melfas Firmware Update
#ifdef LGE_MELFAS_UPDATE_ENABLE
void display_update_msg(int nMsg)
{
	struct vc_data* vc;
	...
	case 0 : // intro
		...
		fbcon_putcs_byLGE (vc,"F / W   U p d a t e   w i l l   t a k e   a b o u t   40 sec . ",33,2,0);
	break;
	case 1 : // success
