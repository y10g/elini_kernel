...
extern void mcs6000_info(unsigned char* fw_ver, unsigned char* hw_ver);
						 ...
						 void (*update_atcmd_state)(char *cmd, int state) )
{
  ...
  char *argv[] = {
	...
	case ATCMD_MMCFORMAT:  // 129
		...
		break;
/*LGE_CHANGE_S,DCM_JP[heari.kim@lge.com,10/11/22,< > Elini]*/		
        case ATCMD_TOUCH_UPGRADE:
                if(at_act != ATCMD_ACTION)
                result = HANDLE_FAIL;
                update_atcmd_state("touch", at_param); //state is up? down?
			...
			switch(at_param)
			{
			    ...
			    case 1:				
				  if ((ret =     call_usermodehelper(argv[0], argv, envp, UMH_WAIT_PROC)) != 0) {
					printk(KERN_ERR "[touch]touch upgrade  failed to run : %i\n", ret);
					}
				break;
			    case 2://version info				  
					...
					ret_value1=fw_ver;
					ret_value2=hw_ver;
				//execve("/system/bin/tsdown", "fwver",5);	
				break;				
			}			
		break;
